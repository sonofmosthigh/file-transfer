...
//function to get client ip address and port
int get_client_details(char *str, char *client_ip, int *client_port){
...
int data_connection(int *fd, char *client_ip, int client_port, int server_port){

	struct sockaddr_in client_addr, tempaddr;
	...
	while(1){
		...
		if((pid = fork()) == 0){
			...
			while(1){
    			...
    			get_client_details(recvline, client_ip, &client_port);

    			if((data_connection(&datafd, client_ip, client_port, port)) < 0){
    				break;
    			}
